...
Our `createStore` is already pretty complete, but there's one more enhancement that's **incredibly useful** in real-world applications.

Sometimes a store owns an external resource that shouldn't stay alive forever.
For example, it might open a WebSocket connection, subscribe to browser events, or start a polling timer.
...
If nobody is subscribed to the store, keeping the WebSocket connected is just wasting resources.\
Instead, we can:
...
```ts title="store.ts" ins={5-8,25-27,31-33}
type Subscriber<T> = (newValue: T, previousValue: T) => void;
...
This small enhancement makes the store much more practical without changing its core design.
...
- Finally, we added lifecycle hooks for real-world use cases.
